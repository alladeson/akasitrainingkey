            ...
            <div class="row">
                <div class="col-xl-12">
                    <div class="hero-course-1-text">
                        <h2>{{__("catalog.key1") }}</h2>
                    </div>
                    <div class="course-title-breadcrumb">
                        <nav>
                            <ol class="breadcrumb" id="courses-catalog">
                                <li class="breadcrumb-item"><a href="{{ route('home') }}">{{__("catalog.key2") }}</a></li>
                                <li class="breadcrumb-item"><span>{{__("catalog.key3") }}</span></li>
                            </ol>
                        </nav>
                    </div>
                </div>
...
@section('third_party_scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Auto filter for Webinars
            if({{$webinar}}) {
                $('input#webinar').prop('checked', true);
                learning_mode.push('Webinar');
                filter_items.push('webinar');
                formatFilterItems()
            }
        });
    </script>
    ...
    <script>
        function getCourses(event) {
            ...
            $.ajax({
                url: '{{ route('courses.catalog.list') }}',
                method: "POST",
                data: {
                    search: $('#search').val(),
                ...
                complete: function() {
                    setTimeout(function() {
                        ...
                        $('div.item-total-number-showing').find('h3.courses-number').text($('#total-number').val() + ' courses');
                    }, 2000);
                },
            });
        }
